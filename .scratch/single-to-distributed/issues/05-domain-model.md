...
## 보충 4 (2026-09-05, [어드민 계정 초기화 규칙] 해결 중)

**unique 제약 목록에 `users(email)`이 빠져 있었다.** 팔로우·구독·플레이리스트-콘텐츠·콘텐츠 외부 ID·리뷰 다섯 개만 적혀 있다.

- **`users(email)`** — 사용자당 이메일 1개

너무 당연해서 빠진 것으로 보이지만, [어드민 계정 초기화 규칙]이 **인스턴스 2대 동시 기동의 경합을 정확히 이 제약에 맡긴다.** 명시적으로 적어둔다. 제약이 없으면 어드민이 두 개 생기고, 그 티켓의 Q3이 근거를 잃는다.

계약의 `POST /api/users`도 이메일 중복 가입을 거부해야 하므로 애플리케이션 레벨 검사와 별개로 DB 제약이 필요하다.
