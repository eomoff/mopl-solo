...
## 보충 3 (2026-09-03, [콘텐츠 수집 배치 설계] 해결 중)

**테이블 둘이 추가됐다.**

- **수집 워터마크** — 소스·축별 마지막 성공 시각. 배치 메타데이터가 아니라 **도메인 상태**이므로 Spring Batch의 `ExecutionContext`가 아닌 별도 테이블에 둔다. TMDB는 익스포트 축과 `/changes` 축의 기준점이 달라 **워터마크를 따로 관리한다.**
- **묘비(tombstone)** — `(source, externalId)`. 관리자가 지운 콘텐츠를 배치가 되살리지 않도록 재수집에서 제외한다.

**콘텐츠 제목은 수집 시점에 폴백 체인(`ko → en → original_*`)을 적용한 단일 값으로 저장한다.** `title`이 `keywordLike` 검색과 정렬의 대상이므로 조회 시점 분기가 불가능하다.

**Spring Batch 메타데이터 테이블은 서비스 DB에 함께 둔다.**
