...
- **스키마 관리 방식** — `ddl-auto`로 갈 것인가 마이그레이션 도구(Flyway 등)를 들일 것인가. [어드민 계정 초기화 규칙]을 풀다 **지도 어디에도 이 결정이 없다는 것이 드러났다.** 그 티켓은 "어드민 초기화에 Flyway를 쓰지 않는다"까지만 답했다. AWS에서 RDS를 스냅샷 후 삭제·복원하는 [AWS 배포 형상 확정]의 형상과, 전환마다 PR 하나인 [브랜치 전략과 CI/CD 결정]이 함께 걸린다.
