...
- [브랜치 전략과 CI/CD 결정](issues/12-ci-cd.md): 기능 브랜치 + PR, **승인 요구 없이 필수 상태 검사만** (혼자인데 승인을 요구하면 매번 우회하게 된다). **이음매 전환마다 PR 하나** = 되돌릴 수 있는 단위. CI는 `./gradlew build` 하나로 시작해 전환 시점에 단일/분산 job으로 분리. 배지는 Gist + shields.io, 배포는 **`workflow_dispatch` 수동 + OIDC**(장기 키를 시크릿에 넣지 않는다). 80% 게이트는 처음부터 켜되 `dto`·`config`·`MoplApplication` 제외. **(2026-09-05 수정: 봇 리뷰어를 붙이지 않고 PR 전 로컬 `/code-review`로 대체. 이슈를 만들지 않으므로 브랜치는 `<타입>/<슬러그>` — 규칙은 [`CONTRIBUTING.md`](../../CONTRIBUTING.md))**
...
- [로깅 정책 확정](issues/20-logging.md): 상관관계 ID는 **도구를 들이지 않고 MDC 필터로**(엣지의 `X-Request-ID`를 이어받는다) — 대가는 **비동기 경계 다섯 곳**이고 각 기능 주차에 하나씩 붙인다. 로컬 평문·배포 JSON, 요청 로그는 **본문 없이**. 레벨 기준은 **"사람이 개입해야 하는가"** 이고 **4xx는 `ERROR`가 아니다**(계약이 중복 가입도 400으로 내보내므로 정상적인 실수가 오류 지표를 덮는다). 예외는 핸들러 한 곳, 스택트레이스는 5xx에만. **WS·SSE 연결을 `INFO`로 남기고 로그에 인스턴스 식별자를 고정 필드로** — 6주차 전환의 증거가 된다.

## Not yet specified

*(없음.)* **이 지도의 모든 티켓이 닫혔고 안개가 남지 않았다 — 목적지에 도달했다.**
...
- **Actuator 엔드포인트 노출 범위** — [로깅 정책 확정]을 풀다 드러났다. [콘텐츠 수집 배치 설계]가 메트릭 4종을 노출하기로 했으나 인가를 정하지 않았고, [인증·인가와 토큰 무효화 설계]도 actuator 경로를 다루지 않았다. **결정이라기보다 적용에 가까워** 목적지 밖으로 둔다 — 답이 사실상 정해져 있고([인증·인가와 토큰 무효화 설계]가 세운 인가 구조에 경로 하나를 얹는 일), 구현 시점에 판단한다. 구현자가 헤매지 않도록 기본선은 [인증·인가와 토큰 무효화 설계]에 보충으로 적어뒀다.