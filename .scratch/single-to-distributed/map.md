...
- **Actuator 엔드포인트 노출 범위와 인가** — [콘텐츠 수집 배치 설계]가 커스텀 메트릭 4종을 Actuator로 노출하기로 했으나 **누가 볼 수 있는지가 없다.** [로깅 정책 확정]을 풀다 드러났다. ECS와 Nginx가 헬스체크를 하려면 `/actuator/health`가 인증 없이 열려야 하는데, `/actuator/metrics`가 함께 열리면 public 인터넷에 내부 지표가 노출된다. [인증·인가와 토큰 무효화 설계]가 SecurityFilterChain을 설계했으나 actuator 경로는 다루지 않았다.
