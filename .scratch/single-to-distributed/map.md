...
- **캐시 전략** — 무엇을 캐시하고 무효화를 어떻게 할 것인가. 이음매와 도메인 모델이 정해져야 질문이 선명해진다.
- **AWS 배포 형상** — ECS 태스크 구성, Nginx 리버스 프록시 배치, 이미지 빌드·배포 경로. 그리고 "내렸다 올려도 보존되어야 하는 상태"가 정확히 무엇인지.
- **테스트 전략의 구체** — Testcontainers를 어디까지 쓸지, 전환 이음매를 감싸는 테스트가 어떤 모양이어야 갈아끼우기를 지켜주는지.
...
- **분산 전환의 절차** — 단일에서 분산으로 넘어갈 때 데이터를 옮기는지, 무중단이 필요한지, 그냥 새로 띄우는지.
